...
#SBATCH --output=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-oracle-eval-%j.out
#SBATCH --error=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-oracle-eval-%j.err
set -euo pipefail
...
OUT=$VLM4WAM_ROOT/eval_results_oracle_iter3000_$(date +%Y%m%d_%H%M%S)
...
EPISODES=(
  "episode_000001_left_external 0"
  "episode_000003_left_external 0"
  "episode_000004_left_external 6"
  "episode_000007_left_external 0"
  "episode_000012_left_external 8"
  "episode_000015_left_external 0"
)

SAMPLES=()
for entry in "${EPISODES[@]}"; do
  set -- $entry; stem=$1; start=$2
  video="$DR/videos/$stem.mp4"
  cap="$DR/metas/$stem.txt"
