...
CALLER_PWD="$PWD"
CONFIG="${CONFIG:-${1:-$GE_ACT_ROOT/configs/ltx_model/libero/action_model_libero_baton_stage2_hdf5.yaml}}"
if [[ "$CONFIG" != /* ]]; then
...
cd "$GE_ACT_ROOT"
RESOLVED_CONFIG="$(mktemp "${TMPDIR:-/tmp}/ge-act-baton-stage2.XXXXXX.yaml")"
cleanup() {
