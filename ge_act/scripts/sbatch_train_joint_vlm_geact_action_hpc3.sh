...
if [[ ! -f "$CONFIG" ]]; then
    echo "joint action config does not exist: $CONFIG" >&2
    exit 2
fi

MAIN_ARGS=(--config_file "$CONFIG")
if [[ "$RUN_KIND" == "smoke8" ]]; then
    SMOKE_OUTPUT_DIR=${SMOKE_OUTPUT_DIR:-/data/user/jhe724/junjie/outputs/smoke_joint_vlm_geact_action_${SLURM_JOB_ID:-manual}}
    MAIN_ARGS+=(
        --max_train_steps 2
