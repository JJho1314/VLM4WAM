#!/usr/bin/env bash
#SBATCH --job-name=geact-baton-s2
...
#SBATCH --output=logs/slurm-geact-baton-s2-%j.out
...
CONFIG="${CONFIG:-$GE_ACT_ROOT/configs/ltx_model/libero/action_model_libero_baton_stage2_hdf5.yaml}"

mkdir -p "$GE_ACT_ROOT/logs"
cd "$GE_ACT_ROOT"
bash scripts/train_ltx_baton_stage2.sh "$CONFIG"
