...
class Trainer:
    ...
    def prepare_optimizer(self):
        ...
        scheduler_warmup_steps = (
            self.args.lr_warmup_steps * self.state.accelerator.num_processes
        )
        scheduler_training_steps = (
            self.state.train_steps * self.state.accelerator.num_processes
        )
        if self.args.lr_scheduler == "cosine_with_min_lr":
            lr_scheduler = build_cosine_with_min_lr_scheduler(
                optimizer,
                num_warmup_steps=scheduler_warmup_steps,
                num_training_steps=scheduler_training_steps,
                min_lr=float(self.args.lr_min),
