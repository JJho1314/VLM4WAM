...
PROJECT_ROOT=${PROJECT_ROOT:-/data/LFT-W02_data/junjie/VLA_WM/VLM4WAM}
cd "$REPO_ROOT" || exit 2
...
RAW_FEATURE_DIR=${RAW_FEATURE_DIR:-$DATASET_DIR/target_features_rawseg_ft}
DENSE_FEATURE_DIR=${DENSE_FEATURE_DIR:-$DATASET_DIR/target_features_instructsam_decoder_dense_stage2_lora_green_leaf_prompt}
CHECKPOINT=${CHECKPOINT:-$PROJECT_ROOT/checkpoints/match_ground_v3_spatial_decoder_dense_93f_s1_iter_000000400}
EXPERIMENT=${EXPERIMENT:-predict2_video2world_training_2b_droid_success_v21_match_ground_v3_spatial_decoder_dense}
RUN_ROOT=${RUN_ROOT:-$REPO_ROOT/outputs/eval_match_ground_v3_spatial_iter400_yellow_carrot_green_leaf_local_$(date +%Y%m%d_%H%M%S)}
OUTPUT_PREFIX=${OUTPUT_PREFIX:-match_ground_v3_spatial_iter400_yellow_carrot_green_leaf}
SEED=${SEED:-20260613}
...
for name, path, dim in (
    ...
    print(name, path, tuple(feat.shape), "feature_mode=", obj.get("feature_mode") if isinstance(obj, dict) else None)
    if feat.shape[-1] != dim:
        raise ValueError(f"{name}: expected dim {dim}, got {feat.shape[-1]}")
PY
