#!/usr/bin/env bash
# Train MASK-FREE what/where target context:
#   target_feature       = InstructSAM raw [SEG]/[TGT] hidden state (what)
#   target_dense_feature = InstructSAM decoder_dense spatial map (where)
# The fused tokens enter a separate small-gated target cross-attention branch;
# no mask or target feature is written directly into video latents.

#SBATCH --job-name=cosmos-wwctx
...
#SBATCH --output=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-wwctx-%j.out
#SBATCH --error=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-wwctx-%j.err
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-what_where_context}
...
export IMAGINAIRE_OUTPUT_ROOT=${IMAGINAIRE_OUTPUT_ROOT:-/data/user/jhe724/workspace/cosmos-predict2.5/outputs/droid_v21_what_where_context_49f_s123_vlm4wam}
...
md5sum \
  cosmos_predict2/_src/predict2/models/video2world_model.py \
...
EXPERIMENT=${EXPERIMENT:-predict2_video2world_training_2b_droid_success_v21_what_where_context}
JOB_NAME=${JOB_NAME:-2b_what_where_context_iou50_49f_s123_bs1accum16_gbs128_1600}
