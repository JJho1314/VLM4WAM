#!/usr/bin/env bash
# Visualize target-matching logits for the mask-free all-block context run.
# The ground-truth target mask is used only for overlay and metrics, not as a
# conditioning input to Cosmos.

#SBATCH --job-name=mgv3match-viz
...
#SBATCH --output=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-mgv3match-viz-%j.out
#SBATCH --error=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-mgv3match-viz-%j.err
...
if [ ! -f "$REPO_ROOT/scripts/visualize_target_matching_logits.py" ]; then
  echo "Invalid REPO_ROOT=${REPO_ROOT}; matching-logits visualization script not found." >&2
  exit 2
fi
cd "$REPO_ROOT"
...
MATCH_VIZ_OUT=${MATCH_VIZ_OUT:-$VLM4WAM_ROOT/attention_vis/${JOB_NAME}_${CHECKPOINT}_${VIZ_SPLIT}_matching_logits}
...
echo "match_viz_out=$MATCH_VIZ_OUT"
echo "split=$VIZ_SPLIT samples=$VIZ_SAMPLES max_batches=$VIZ_MAX_BATCHES token_source=$VIZ_TOKEN_SOURCE"

nvidia-smi -L
python -c "import torch; print('cuda count:', torch.cuda.device_count())"

torchrun --standalone --nproc_per_node=8 -m scripts.visualize_target_matching_logits \
  --config=cosmos_predict2/_src/predict2/configs/video2world/config.py \
  --checkpoint="$CHECKPOINT" \
  --output-dir="$MATCH_VIZ_OUT" \
  --split="$VIZ_SPLIT" \
...
status=$?
echo "matching_viz_exit=$status"
echo "summary=$MATCH_VIZ_OUT/target_matching_logits_summary.json"
exit "$status"
