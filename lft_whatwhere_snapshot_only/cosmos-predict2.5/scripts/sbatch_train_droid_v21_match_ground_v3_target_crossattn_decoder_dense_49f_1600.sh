#!/usr/bin/env bash
# Train MASK-FREE match-ground v3 with OmniWeaving-style target cross-attention.
# I2V setting: 49 frames, stride choices 1/2/3, 1600 optimizer steps.

#SBATCH --job-name=cosmos-mgv3txa49
...
#SBATCH --output=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-mgv3txa49-%j.out
#SBATCH --error=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-mgv3txa49-%j.err
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-match_ground_v3_target_crossattn_decoder_dense}
...
export IMAGINAIRE_OUTPUT_ROOT=${IMAGINAIRE_OUTPUT_ROOT:-/data/user/jhe724/workspace/cosmos-predict2.5/outputs/droid_v21_match_ground_v3_target_crossattn_decoder_dense_49f_s123_vlm4wam}
...
EXPERIMENT=${EXPERIMENT:-predict2_video2world_training_2b_droid_success_v21_match_ground_v3_target_crossattn_decoder_dense}
JOB_NAME=${JOB_NAME:-2b_mgv3_target_crossattn_decoder_dense_iou50_49f_s123_bs2accum8_gbs128_1600}
...
echo "=== TRAIN match-ground v3 + target cross-attn decoder-dense; experiment=${EXPERIMENT}; frames=${DROID_SUCCESS_V21_TAVID_NUM_FRAMES}; strides=${DROID_SUCCESS_V21_TAVID_FRAME_STRIDES}; per_gpu_batch=${BATCH_SIZE}; grad_accum=${GRAD_ACCUM_ITER}; global_batch=$((BATCH_SIZE * 8 * GRAD_ACCUM_ITER)); max_iter=${MAX_ITER}; job_name=${JOB_NAME}; output_root=${IMAGINAIRE_OUTPUT_ROOT} ==="
