#!/usr/bin/env bash
# Train MASK-FREE match-ground v3 with spatial-positioned InstructSAM dense
# context and a zero-gated target cross-attention branch.
# I2V setting: 49 frames, stride choices 1/2/3, 2000 optimizer steps.

#SBATCH --job-name=cosmos-mgv3ctxsp
...
#SBATCH --output=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-mgv3ctxsp-%j.out
#SBATCH --error=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-mgv3ctxsp-%j.err
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-match_ground_v3_target_context_spatialpos_crossattn}
...
echo "=== TRAIN match-ground v3 + spatial-pos dense target context + xattn; experiment=${EXPERIMENT}; frames=${DROID_SUCCESS_V21_TAVID_NUM_FRAMES}; strides=${DROID_SUCCESS_V21_TAVID_FRAME_STRIDES}; per_gpu_batch=${BATCH_SIZE}; grad_accum=${GRAD_ACCUM_ITER}; global_batch=$((BATCH_SIZE * 8 * GRAD_ACCUM_ITER)); max_iter=${MAX_ITER}; job_name=${JOB_NAME}; output_root=${IMAGINAIRE_OUTPUT_ROOT} ==="
