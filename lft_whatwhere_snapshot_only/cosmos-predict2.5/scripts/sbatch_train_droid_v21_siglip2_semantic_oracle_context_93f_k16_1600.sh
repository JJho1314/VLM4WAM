#!/usr/bin/env bash
# Oracle test: train Cosmos to consume longer GT future SigLIP2 semantic blueprints.
# This variant targets complete manipulation clips with 93 generated frames and
# 16 semantic keyframes, following the Plan-X/Baton sparse-keyframe idea.
#
# No explicit target mask is provided to Cosmos.

#SBATCH --job-name=cosmos-siglip2-93f-k16
...
#SBATCH --output=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-siglip2-93f-k16-%j.out
#SBATCH --error=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-siglip2-93f-k16-%j.err
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-siglip2_semantic_oracle_context_93f_k16}
...
export DROID_SUCCESS_V21_TAVID_NUM_FRAMES=${DROID_SUCCESS_V21_TAVID_NUM_FRAMES:-93}
export DROID_SUCCESS_V21_TAVID_FRAME_STRIDES=${DROID_SUCCESS_V21_TAVID_FRAME_STRIDES:-2,3}
export DROID_SUCCESS_V21_TAVID_FRAME_START_POLICY=${DROID_SUCCESS_V21_TAVID_FRAME_START_POLICY:-range_start}
export IMAGINAIRE_OUTPUT_ROOT=${IMAGINAIRE_OUTPUT_ROOT:-/data/user/jhe724/workspace/cosmos-predict2.5/outputs/droid_v21_siglip2_semantic_oracle_context_320_93f_s23_k16_vlm4wam}
export TOKENIZERS_PARALLELISM=false
export NCCL_DEBUG=WARN
export PYTORCH_CUDA_ALLOC_CONF=${PYTORCH_CUDA_ALLOC_CONF:-expandable_segments:True}

FEATURE_DIR_NAME=${FEATURE_DIR_NAME:-siglip2_semantic_plan_k16_g9_full}
NUM_KEYFRAMES=${NUM_KEYFRAMES:-16}
GRID_SIZE=${GRID_SIZE:-9}
TARGET_FEATURE_MAX_TOKENS=${TARGET_FEATURE_MAX_TOKENS:-$((NUM_KEYFRAMES * GRID_SIZE * GRID_SIZE))}
TARGET_FEATURE_DIM=${TARGET_FEATURE_DIM:-1152}
export FEATURE_DIR_NAME NUM_KEYFRAMES GRID_SIZE TARGET_FEATURE_MAX_TOKENS TARGET_FEATURE_DIM
...
feature_dir = os.environ.get("FEATURE_DIR_NAME", "siglip2_semantic_plan_k16_g9_full")
labels = d / feature_dir
...
print(f"{feature_dir}: missing={len(missing)} frame_ranges={len(active_stems)} ({frac:.4f})")
if missing:
...
expected = (int(os.environ.get("NUM_KEYFRAMES", "16")), int(os.environ.get("GRID_SIZE", "9")) ** 2, 1152)
assert tuple(plan.shape) == expected, (tuple(plan.shape), expected)
PY
...
JOB_NAME=${JOB_NAME:-2b_siglip2_semantic_oracle_context_iou50_320_93f_s23_k16_b2accum8_gbs128_1600}
...
torchrun --standalone --nproc_per_node=8 -m scripts.train \
  ...
  dataloader_train.dataset.target_feature_dir="$FEATURE_DIR_NAME" \
  dataloader_train.sampler.dataset.target_feature_dir="$FEATURE_DIR_NAME" \
  dataloader_val.dataset.target_feature_dir="$FEATURE_DIR_NAME" \
  dataloader_val.sampler.dataset.target_feature_dir="$FEATURE_DIR_NAME" \
  dataloader_train.dataset.target_feature_dim="$TARGET_FEATURE_DIM" \
  dataloader_train.sampler.dataset.target_feature_dim="$TARGET_FEATURE_DIM" \
  dataloader_val.dataset.target_feature_dim="$TARGET_FEATURE_DIM" \
  dataloader_val.sampler.dataset.target_feature_dim="$TARGET_FEATURE_DIM" \
  dataloader_train.dataset.target_feature_max_tokens="$TARGET_FEATURE_MAX_TOKENS" \
  dataloader_train.sampler.dataset.target_feature_max_tokens="$TARGET_FEATURE_MAX_TOKENS" \
  dataloader_val.dataset.target_feature_max_tokens="$TARGET_FEATURE_MAX_TOKENS" \
  dataloader_val.sampler.dataset.target_feature_max_tokens="$TARGET_FEATURE_MAX_TOKENS" \
  ...
  model.config.net.target_feature_context_max_tokens="$TARGET_FEATURE_MAX_TOKENS" \
