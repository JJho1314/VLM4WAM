#!/usr/bin/env bash
# Evaluate the mask-free what/where InstructSAM context checkpoint.
# Generation removes target masks from Cosmos input.  Masks are used only by
# the offline analysis script to measure target-vs-background differences.

#SBATCH --job-name=wwctx1600eval
...
#SBATCH --output=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-wwctx1600eval-%j.out
#SBATCH --error=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-wwctx1600eval-%j.err
...
IMAGINAIRE_OUTPUT_ROOT=${IMAGINAIRE_OUTPUT_ROOT:-/data/user/jhe724/workspace/cosmos-predict2.5/outputs/droid_v21_what_where_context_49f_s123_vlm4wam}
...
RUN_ROOT=${RUN_ROOT:-$VLM4WAM_ROOT/feature_guidance_analysis/what_where_context_iter1600_yellow_carrot_${TIMESTAMP}}
OUTPUT_PREFIX=${OUTPUT_PREFIX:-what_where_context_iter1600_yellow_carrot}
...
summary = {
    ...
    "experiment": "$EXPERIMENT",
    "job_name": "$JOB_NAME",
    "video_size": [int("$VIDEO_SIZE_H"), int("$VIDEO_SIZE_W")],
