#!/usr/bin/env bash
# Train MASK-FREE what/where target context with an internal soft where prior:
#   target_feature       = InstructSAM raw [SEG]/[TGT] hidden state (what)
#   target_dense_feature = InstructSAM decoder_dense spatial map (where)
#   prior_head           = target_dense_feature -> soft objectness gate
# Cosmos inference receives no explicit mask; masks supervise only the internal
# prior/attention losses during training.

#SBATCH --job-name=cosmos-wwprior
...
#SBATCH --output=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-wwprior-%j.out
#SBATCH --error=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-wwprior-%j.err
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-what_where_prior_context}
...
export IMAGINAIRE_OUTPUT_ROOT=${IMAGINAIRE_OUTPUT_ROOT:-/data/user/jhe724/workspace/cosmos-predict2.5/outputs/droid_v21_what_where_prior_context_49f_s123_vlm4wam}
...
VIDEO_SIZE_H=${VIDEO_SIZE_H:-480}
VIDEO_SIZE_W=${VIDEO_SIZE_W:-864}
EXPERIMENT=${EXPERIMENT:-predict2_video2world_training_2b_droid_success_v21_what_where_prior_context}
JOB_NAME=${JOB_NAME:-2b_what_where_prior_context_iou50_49f_s123_bs2accum8_gbs128_3000}
...
echo "=== TRAIN what/where PRIOR target-branch context; experiment=${EXPERIMENT}; video_size=${VIDEO_SIZE_H}x${VIDEO_SIZE_W}; frames=${DROID_SUCCESS_V21_TAVID_NUM_FRAMES}; strides=${DROID_SUCCESS_V21_TAVID_FRAME_STRIDES}; per_gpu_batch=${BATCH_SIZE}; grad_accum=${GRAD_ACCUM_ITER}; global_batch=$((BATCH_SIZE * 8 * GRAD_ACCUM_ITER)); max_iter=${MAX_ITER}; job_name=${JOB_NAME}; output_root=${IMAGINAIRE_OUTPUT_ROOT} ==="
torchrun --standalone --nproc_per_node=8 -m scripts.train \
  ...
  dataloader_val.dataset.video_size="[$VIDEO_SIZE_H,$VIDEO_SIZE_W]" \
  dataloader_val.sampler.dataset.video_size="[$VIDEO_SIZE_H,$VIDEO_SIZE_W]" \
