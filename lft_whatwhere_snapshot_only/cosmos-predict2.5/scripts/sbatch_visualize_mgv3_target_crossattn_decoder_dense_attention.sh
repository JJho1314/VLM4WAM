#!/usr/bin/env bash
# Visualize intermediate target-cross-attention for the match-ground-v3
# decoder-dense Target-CrossAttn run.  This exports both paper-style figures
# and raw debug packs (attention tensors + per-frame metrics).

#SBATCH --job-name=mgv3-txa-viz
...
#SBATCH --output=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-mgv3-txa-viz-%j.out
#SBATCH --error=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-mgv3-txa-viz-%j.err
...
VIZ_MAX_BATCHES=${VIZ_MAX_BATCHES:-120}
...
VIZ_OUT=${VIZ_OUT:-$VLM4WAM_ROOT/attention_vis/${JOB_NAME}_${CHECKPOINT}_${VIZ_SPLIT}}
...
torchrun --standalone --nproc_per_node=8 -m scripts.visualize_tavid_cross_attention \
  ...
  --sample-label="target_crossattn_${CHECKPOINT}" \
  --save-debug-pack \
