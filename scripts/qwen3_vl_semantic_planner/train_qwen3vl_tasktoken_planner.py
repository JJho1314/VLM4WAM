#!/usr/bin/env python3
"""Fine-tune Qwen3-VL-Instruct as a continuous semantic planner — TASKTOKEN variant.

Independent fork of train_qwen3vl_semantic_planner.py that adds the lingbot-vla-v2-style
``tasktoken`` plan head (TaskTokenResamplerPlanHead): a learnable per-grid query bank that
cross-attends to a RICH context = [ LM latent hiddens (+) the LLM's own image-token hiddens ],
instead of the CoVT head's queries reading ONLY the compact latents. The image tokens keep 2-D
spatial detail, targeting the CoVT head's weak per-token identity (low retrieval, ~0.64x norm).

Kept fully separate from train_qwen3vl_semantic_planner.py (the SigLIP CoVT production script) on
purpose — the two evolve independently. Run with ``--plan-head-type tasktoken`` (the default here).
Switch the base to 4B by pointing ``--model-path`` at a Qwen3-VL-4B checkpoint; hidden_size auto-detects.
NOTE: this variant still predicts the SAME target feature space (e.g. SigLIP) as the CoVT script, so
it stays compatible with the current WM. Retargeting to DINO-video (lingbot's target) additionally
requires a matching DINO-video-conditioned WM and is out of scope for this file.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--plan-head-type", choices=["mlp", "baton_crossattn", "covt", "tasktoken"], default="tasktoken")
...
class TaskTokenResamplerPlanHead(nn.Module):
    """lingbot-vla-v2-style rich-KV align head (adapted from their TaskTokenResampler, Apache-2.0).

    The CoVT head has a learnable grid query that reads ONLY the LM's compact latents (a hard
    4-latent-per-keyframe bottleneck) -> weak per-token identity (low retrieval, ~0.64x norm).
    Here the same per-grid learnable query bank instead cross-attends to a RICH context:
    ``[ LM latent hiddens  (+)  the LLM's own image-token hiddens ]`` through a small Perceiver
    stack. The image tokens keep 2-D spatial detail, so each query can pull the feature for its
    own cell rather than reconstruct the whole grid from 4 latents. Predicts the same dense
    per-keyframe grid target (e.g. SigLIP 27x27), so it is a drop-in alternative head — the WM
    side is unchanged as long as the target feature space is unchanged.

    v1 assumes every batch item has the same number of image tokens (true when first frames share
    a resolution); it raises otherwise.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if hidden_size % num_heads != 0:
            raise ValueError(f"hidden_size={hidden_size} must be divisible by num_heads={num_heads}")
        if grid_size <= 0:
            raise ValueError(f"tasktoken head needs a positive target grid_size, got {grid_size}")
        self.num_keyframes = int(num_keyframes)
        self.num_latent_per_keyframe = int(num_latent_per_keyframe)
        self.grid_size = int(grid_size)
        self.grid_tokens = self.grid_size * self.grid_size
        self.grid_query = nn.Parameter(torch.randn(self.grid_tokens, hidden_size) * hidden_size ** -0.5)
        self.keyframe_embed = nn.Parameter(torch.zeros(self.num_keyframes, hidden_size))
        self.lat_proj = nn.Linear(hidden_size, hidden_size)   # latent tokens -> KV
        self.img_proj = nn.Linear(hidden_size, hidden_size)   # LLM image tokens -> KV
        self.layers = nn.ModuleList()
        for _ in range(int(num_layers)):
            self.layers.append(
                nn.ModuleList(
                    [
                        nn.LayerNorm(hidden_size),  # query norm
                        nn.LayerNorm(hidden_size),  # context norm
                        nn.MultiheadAttention(hidden_size, num_heads, dropout=dropout, batch_first=True),
                        nn.LayerNorm(hidden_size),  # ffn norm
                        nn.Sequential(
                            nn.Linear(hidden_size, hidden_size * int(ff_mult)),
                            nn.GELU(),
                            nn.Linear(hidden_size * int(ff_mult), hidden_size),
                        ),
                    ]
                )
            )
    ...
    def forward(self, latent_hidden: torch.Tensor, image_hidden: torch.Tensor) -> torch.Tensor:
        # latent_hidden: (B, num_keyframes*num_latent_per_keyframe, H); image_hidden: (B, N_img, H)
        batch = latent_hidden.shape[0]
        hidden = latent_hidden.shape[-1]
        kf = self.num_keyframes
        expected = kf * self.num_latent_per_keyframe
        if latent_hidden.shape[1] != expected:
            raise RuntimeError(f"tasktoken head expected {expected} latent tokens, got {latent_hidden.shape[1]}")
        lat_kv = self.lat_proj(latent_hidden.reshape(batch * kf, self.num_latent_per_keyframe, hidden))
        img_kv = self.img_proj(image_hidden)  # (B, N_img, H)
        n_img = img_kv.shape[1]
        img_kv = img_kv.unsqueeze(1).expand(batch, kf, n_img, hidden).reshape(batch * kf, n_img, hidden)
        context = torch.cat([img_kv, lat_kv], dim=1)  # (B*kf, N_img + npl, H)
        query = self.grid_query.unsqueeze(0) + self.keyframe_embed.unsqueeze(1)  # (kf, grid, H)
        query = query.unsqueeze(0).expand(batch, -1, -1, -1).reshape(batch * kf, self.grid_tokens, hidden)
        query = query.to(context.dtype)
        for q_norm, c_norm, attn, ff_norm, ff in self.layers:
            attn_out, _ = attn(q_norm(query), c_norm(context), c_norm(context), need_weights=False)
            query = query + attn_out
            query = query + ff(ff_norm(query))
        query = self.out_norm(query)
        out = self.sem_mlp(query)  # (B*kf, grid, semantic_dim)
        return out.reshape(batch, kf * self.grid_tokens, out.shape[-1])
...
class PlannerWrapper(nn.Module):
    def __init__(
        ...
        elif plan_head_type == "tasktoken":
            # lingbot-vla-v2-style rich-KV head: same distinct-latent bottleneck as covt, but the
            # grid queries also read the LLM image tokens (see TaskTokenResamplerPlanHead).
            self.latent_len = int(num_keyframes) * int(num_latent_per_keyframe)
            self.plan_head = TaskTokenResamplerPlanHead(
                num_keyframes=num_keyframes,
                num_latent_per_keyframe=num_latent_per_keyframe,
                grid_size=grid_size,
                hidden_size=hidden_size,
        ...
        self.model = model
        # image-token id used by the tasktoken head to gather the LLM's image-token hiddens
    ...
    def collect_image_hidden(self, hidden: torch.Tensor, input_ids: torch.Tensor) -> torch.Tensor:
        """Gather the LLM's image-token hidden states (KV context for the tasktoken head)."""
        if self.image_token_id is None:
            raise RuntimeError("tasktoken head needs model.config.image_token_id, which is unset")
        mask = input_ids == int(self.image_token_id)
        counts = mask.sum(dim=1)
        n_img = int(counts[0].item())
        if n_img == 0 or not bool(torch.all(counts == counts[0])):
            raise RuntimeError(
                f"tasktoken head v1 needs an equal, nonzero image-token count per batch item, got {counts.tolist()}"
            )
    ...
    def predict_semantic_plan(self, **inputs: Any) -> torch.Tensor:
        ...
        if self.plan_head_type == "tasktoken":
            image_hidden = self.collect_image_hidden(hidden, input_ids)
            return self.plan_head(plan_hidden.to(dtype=head_dtype), image_hidden.to(dtype=head_dtype)).float()
...
def main() -> None:
    ...
    if args.plan_head_type in ("covt", "tasktoken"):
