#!/usr/bin/env python3
"""Per-camera SQUARE viz for the 2B DINOv3+DA3 LIBERO planner (main & wrist shown separately).

Features/depth are computed on the full [224,448] 2-cam composite (unchanged); each rendered
panel is un-squished to [224,448] then split at the horizontal midpoint into two 224x224 squares.
Two PNGs per sample: sample_XX_main.png / sample_XX_wrist.png, each a 3x6 grid:
  Row0 DINO hi-res PCA | Row1 depth probe_v2 (turbo) | Row2 DA3-full GT depth (ref under TARGET cols).
"""
...
from train_feature_probes import FeatureUpsampler, MiniDPTProbe
...
@torch.no_grad()
def hires_pca(ups, tt, tp, device):
    ft = ups(tt.unsqueeze(0).to(device).float())[0]; fp = ups(tp.unsqueeze(0).to(device).float())[0]
    g = ups.out_grid; X = torch.cat([ft,fp],0); Xc = X - X.mean(0,keepdim=True)
    ...
    n=ft.shape[0]; return proj[:n].reshape(g,g,3).cpu().numpy(), proj[n:].reshape(g,g,3).cpu().numpy()
...
def main():
    ...
    ap.add_argument("--dino-up-probe", type=Path, default=Path("/data/users/junjie/probes_2b/dino_upsample_probe.pt"))
    ap.add_argument("--da3-v2-probe", type=Path, default=Path("/data/users/junjie/probes_2b/da3_depth_v2_probe.pt"))
    ap.add_argument("--output-dir", type=Path, default=Path("/data/users/junjie/viz_2b_split"))
    ...
    # trainer's from_exported_checkpoint now validates the plan-token snapshot in fp32 (rtol/atol=2e-2)
    # and only WARNS -> no stub needed.
    ...
    dino=Dinov3TargetEncoder(input_size=int(meta.get("dino_input_size") or 256),device=device)
    ...
    m=full.model
    ups=load(args.dino_up_probe, FeatureUpsampler, device); dprobe=load(args.da3_v2_probe, MiniDPTProbe, device)
    ...
    for i in idxs:
        ...
        with torch.no_grad():
            ...
            cdino_t,fdino_t=dino.encode_current_and_future(cur,kf,effective_fps=efps)
            cdep_t,fdep_t=da3.encode_current_and_future(cur,kf)
            cur_gt=gt_turbo(m,da3,cur,device); fut_gt=gt_turbo(m,da3,kf,device)
        cdino_p,fdino_p=cf(plans["current_dino"]),cf(plans["future_dino"])
        cdep_p,fdep_p=cf(plans["current_depth"]),cf(plans["future_depth"])
        cdino_tt,fdino_tt=cf(cdino_t),cf(fdino_t); cdep_tt,fdep_tt=cf(cdep_t),cf(fdep_t)
        cdino_mse=F.mse_loss(cdino_p,cdino_tt).item(); fdino_mse=F.mse_loss(fdino_p,fdino_tt).item()
        cdep_sl1=F.smooth_l1_loss(cdep_p,cdep_tt).item(); fdep_sl1=F.smooth_l1_loss(fdep_p,fdep_tt).item()
        cdino_tr,cdino_pr=hires_pca(ups,cdino_tt,cdino_p,device); fdino_tr,fdino_pr=hires_pca(ups,fdino_tt,fdino_p,device)
        ...
        # Every panel as [224,448], then split into main/wrist halves.
        full_rows_imgs=[
            [to448(to_disp(cur_img[0])), to448(to_disp(kfs[0,0])), to448(cdino_tr), to448(cdino_pr), to448(fdino_tr), to448(fdino_pr)],
        ...
        titles=[
            ["Current RGB","Future RGB","DINO cur TARGET","DINO cur PRED","DINO fut TARGET","DINO fut PRED"],
        ...
        for cam, sel in (("main", 0), ("wrist", 1)):
            ...
            sup=f"[{cam} cam] {instr[:80]}\ndino_mse cur={cdino_mse:.4f} fut={fdino_mse:.4f}  |  depth_sl1 cur={cdep_sl1:.3f} fut={fdep_sl1:.3f}"
