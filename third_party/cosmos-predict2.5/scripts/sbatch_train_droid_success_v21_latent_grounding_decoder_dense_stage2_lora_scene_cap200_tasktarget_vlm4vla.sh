#!/usr/bin/env bash
# Train mask-free latent-space grounding from InstructSAM decoder dense features.
# Cosmos receives no explicit target mask; target guidance is injected inside DiT
# blocks as a gated latent target field.

#SBATCH --job-name=cosmos-latent-ground
...
#SBATCH --output=/data/user/jhe724/workspace/VLM4VLA/slurm-latent-ground-%j.out
#SBATCH --error=/data/user/jhe724/workspace/VLM4VLA/slurm-latent-ground-%j.err
...
export IMAGINAIRE_OUTPUT_ROOT=${IMAGINAIRE_OUTPUT_ROOT:-$VLM4VLA_ROOT/outputs/droid_success_v21_latent_grounding_decoder_dense_stage2_lora_scene_cap200_tasktarget}
...
export JOB_NAME=${JOB_NAME:-2b_droid_success_v21_latent_grounding_decoder_dense_stage2_lora_scene_cap200_tasktarget_49f_s234_bs2accum8_gbs128_2000step_val400_from_base}

if [ ! -f "$BASE_SCRIPT" ]; then
  echo "Invalid BASE_SCRIPT=${BASE_SCRIPT}; latent-grounding wrapper could not find base training script." >&2
  exit 2
fi
