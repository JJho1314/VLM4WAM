#!/usr/bin/env bash
# Train Cosmos 2B TEXT-FREE on fused multi-source InstructSAM features
# (mask+detect+vtext). T5 text is dropped; the InstructSAM feature is the only
# cross-attention context. Defaults to the cap200_tasktarget holdout.

#SBATCH --job-name=cosmos-isam-tfms
...
#SBATCH --output=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-isam-tfms-%j.out
#SBATCH --error=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-isam-tfms-%j.err
...
# Reuse the existing cosmos uv venv (read-only); override below ensures the code
# imported is THIS repo's, not the editable install that venv points at.
VENV=${COSMOS_VENV:-/data/user/jhe724/workspace/cosmos-predict2.5/.venv}
export VIRTUAL_ENV=$VENV
export PATH=/data/apps/gcc/11.5/bin:$VENV/bin:$PATH
unset PYTHONHOME
# PYTHONPATH entries precede site-packages, so this repo's cosmos_predict2 wins
# over the venv's editable _cosmos_predict2.pth (-> jhe724's other project).
...
# Internal (intranet) wandb server -- same host the InstructSAM jobs log to.
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-instructsam_textfree_multisource}
...
export IMAGINAIRE_OUTPUT_ROOT=${IMAGINAIRE_OUTPUT_ROOT:-/data/user/jhe724/workspace/cosmos-predict2.5/outputs/droid_success_v21_instructsam_textfree_multisource_cap200_tasktarget}
export TOKENIZERS_PARALLELISM=false
export NCCL_DEBUG=WARN
export PYTORCH_CUDA_ALLOC_CONF=${PYTORCH_CUDA_ALLOC_CONF:-expandable_segments:True}

FEATURE_DIR_NAME=${FEATURE_DIR_NAME:-target_features_multisource}
...
feat_name = os.environ.get("FEATURE_DIR_NAME", "target_features_multisource")
# Only the train dir needs features unless validation is on. Tolerate a tiny
# fraction of missing features (the dataset retries on a missing feature).
TOLERATE = 0.01
run_val = os.environ.get("RUN_VALIDATION", "True").lower() in ("1", "true", "yes")
...
def count_active(dataset_dir):
    ...
    features = sorted((dataset_dir / feat_name).glob("*.pt"))
    missing = [p.stem for p in active if not (dataset_dir / feat_name / f"{p.stem}.pt").exists()]
    return len(videos), len(excluded), len(active), len(features), missing


keys = ["DROID_SUCCESS_V21_TAVID_DIR"]
if run_val:
    keys.append("DROID_SUCCESS_V21_TAVID_VAL_DIR")
for key in keys:
    raw, excluded, active, features, missing = count_active(os.environ[key])
    frac = (len(missing) / active) if active else 0.0
    print(key, os.environ[key])
    print("videos_active:", active, "multisource_features:", features, "missing:", len(missing), f"frac={frac:.4f}")
    if missing:
...
JOB_NAME=${JOB_NAME:-2b_droid_success_v21_instructsam_textfree_multisource_cap200_tasktarget_49f_bs2accum4_14k}
...
echo "=== TRAIN text-free multi-source InstructSAM; experiment=${EXPERIMENT}; per_gpu_batch=${BATCH_SIZE}; grad_accum=${GRAD_ACCUM_ITER}; global_batch=$((BATCH_SIZE * 8 * GRAD_ACCUM_ITER)); max_iter=${MAX_ITER}; feature_dir=${FEATURE_DIR_NAME}; job_name=${JOB_NAME}; output_root=${IMAGINAIRE_OUTPUT_ROOT} ==="
