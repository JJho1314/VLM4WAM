#!/usr/bin/env bash
# Train TAVID-like mask-free target feature control-map conditioning.
# Cosmos receives InstructSAM decoder_dense target_feature only. A learned adapter
# compresses the 256-d dense feature grid to one bounded spatial control channel.

#SBATCH --job-name=cosmos-feat-map
...
#SBATCH --output=/data/user/jhe724/workspace/VLM4VLA/slurm-feature-control-map-%j.out
#SBATCH --error=/data/user/jhe724/workspace/VLM4VLA/slurm-feature-control-map-%j.err
...
export IMAGINAIRE_OUTPUT_ROOT=${IMAGINAIRE_OUTPUT_ROOT:-$VLM4VLA_ROOT/outputs/droid_success_v21_feature_control_map_channel_stage2_lora_scene_cap200_tasktarget}
...
export JOB_NAME=${JOB_NAME:-2b_droid_success_v21_feature_control_map_channel_stage2_lora_scene_cap200_tasktarget_49f_s234_bs2accum8_gbs128_2000step_val400_from_base}
