#!/usr/bin/env bash
# Train Cosmos 2B with implicit InstructSAM target features on strict holdout v3.

#SBATCH --job-name=cosmos-isam-v3
...
#SBATCH --output=/data/user/jhe724/workspace/cosmos-predict2.5/slurm-instructsam-v3-%j.out
#SBATCH --error=/data/user/jhe724/workspace/cosmos-predict2.5/slurm-instructsam-v3-%j.err
...
export IMAGINAIRE_OUTPUT_ROOT=${IMAGINAIRE_OUTPUT_ROOT:-/data/user/jhe724/workspace/cosmos-predict2.5/outputs/droid_success_v21_instructsam_feature_context_strict_holdout_v3}
...
JOB_NAME=${JOB_NAME:-2b_droid_success_v21_instructsam_feature_context_strict_v3_49f_s234_bs2accum4_14k_val1000}

echo "=== TRAIN DROID success v21 InstructSAM feature context strict_holdout_v3; experiment=${EXPERIMENT}; per_gpu_batch=${BATCH_SIZE}; grad_accum=${GRAD_ACCUM_ITER}; global_batch=$((BATCH_SIZE * 8 * GRAD_ACCUM_ITER)); max_iter=${MAX_ITER}; validation=${RUN_VALIDATION}; validation_iter=${VALIDATION_ITER}; max_val_iter=${MAX_VAL_ITER}; save_iter=${SAVE_ITER}; sample_iter=${SAMPLE_ITER}; train_workers=${TRAIN_NUM_WORKERS}; val_workers=${VAL_NUM_WORKERS}; attn_query_chunk=${TAVID_ATTN_QUERY_CHUNK_SIZE}; job_name=${JOB_NAME}; wandb_base=${WANDB_BASE_URL} ==="
