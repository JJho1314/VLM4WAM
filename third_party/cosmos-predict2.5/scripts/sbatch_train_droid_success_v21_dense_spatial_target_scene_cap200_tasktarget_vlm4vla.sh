#!/usr/bin/env bash
# Train dense spatial target injection on the DROID success v21 scene-cap200 task-target split.

#SBATCH --job-name=cosmos-dense-target
...
#SBATCH --output=/data/user/jhe724/workspace/VLM4VLA/slurm-dense-spatial-target-%j.out
#SBATCH --error=/data/user/jhe724/workspace/VLM4VLA/slurm-dense-spatial-target-%j.err
...
export IMAGINAIRE_OUTPUT_ROOT=${IMAGINAIRE_OUTPUT_ROOT:-$VLM4VLA_ROOT/outputs/droid_success_v21_dense_spatial_target_scene_cap200_tasktarget}
export EXPERIMENT=${EXPERIMENT:-predict2_video2world_training_2b_droid_success_v21_dense_spatial_target}
# 17,517 active train videos / global batch 128 = ceil(136.85) = 137 steps/epoch.
# Train 10 epochs = 1,370 optimizer steps. Keep per-GPU batch fixed and use
# accumulation for the larger effective batch so the memory profile stays stable.
...
export JOB_NAME=${JOB_NAME:-2b_droid_success_v21_dense_spatial_target_scene_cap200_tasktarget_49f_s234_bs2accum8_gbs128_10ep1370_val274_from_base}

if [ ! -f "$BASE_SCRIPT" ]; then
  echo "Invalid BASE_SCRIPT=${BASE_SCRIPT}; dense spatial target wrapper could not find base training script." >&2
  exit 2
fi
