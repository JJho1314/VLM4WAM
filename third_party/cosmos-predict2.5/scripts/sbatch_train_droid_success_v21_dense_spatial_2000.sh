#!/usr/bin/env bash
# Train Cosmos 2B with DENSE SPATIAL TARGET injection (design D): the InstructSAM
# [SEG] mask embedding painted into the patchified latent tokens via mask-pooled
# occupancy + coords (DenseTargetSpatialAdapter, tanh-gated residual). Text+[TGT]
# and the TAVID attention loss kept; all cross-attn feature paths OFF; no mask
# channel in the latent input. cap200 set, global batch 128, 2000 steps.

#SBATCH --job-name=cosmos-dense2k
...
#SBATCH --output=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-dense2k-%j.out
#SBATCH --error=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-dense2k-%j.err
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-dense_spatial_target}
...
export IMAGINAIRE_OUTPUT_ROOT=${IMAGINAIRE_OUTPUT_ROOT:-/data/user/jhe724/workspace/cosmos-predict2.5/outputs/droid_success_v21_dense_spatial_2000_vlm4wam}
...
# Dense injection trains on GT masks + InstructSAM [SEG] features: require both.
TOLERATE = 0.01
...
miss_feat = [p.stem for p in active if not (d / "target_features" / f"{p.stem}.pt").exists()]
fm = (len(miss_mask) / len(active)) if active else 0.0
ff = (len(miss_feat) / len(active)) if active else 0.0
print(f"train dir={d} active={len(active)} missing_masks={len(miss_mask)} ({fm:.4f}) missing_features={len(miss_feat)} ({ff:.4f})")
...
EXPERIMENT=${EXPERIMENT:-predict2_video2world_training_2b_droid_success_v21_dense_spatial_target}
JOB_NAME=${JOB_NAME:-2b_droid_success_v21_dense_spatial_cap200_49f_bs2accum8_gbs128_2000}
...
echo "=== TRAIN dense spatial target (design D, latent token injection); experiment=${EXPERIMENT}; per_gpu_batch=${BATCH_SIZE}; grad_accum=${GRAD_ACCUM_ITER}; global_batch=$((BATCH_SIZE * 8 * GRAD_ACCUM_ITER)); max_iter=${MAX_ITER}; job_name=${JOB_NAME}; output_root=${IMAGINAIRE_OUTPUT_ROOT} ==="
