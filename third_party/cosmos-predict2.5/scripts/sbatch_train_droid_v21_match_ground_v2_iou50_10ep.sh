#!/usr/bin/env bash
# Train the MASK-FREE match-ground model: WHERE = matching head over the [SEG]
# projection query (GT-mask supervised on all frames), WHAT = raw [SEG] hidden
# states read via gated attention. No mask anywhere at inference.
# cap200 set, FT-extractor dual features, global batch 128, 2000 steps.

#SBATCH --job-name=cosmos-mgv2
...
#SBATCH --output=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-mgv2-%j.out
#SBATCH --error=/data/user/jhe724/workspace/VLM4WAM/logs/slurm-mgv2-%j.err
...
export WANDB_RUN_GROUP=${WANDB_RUN_GROUP:-match_ground_v2}
...
export IMAGINAIRE_OUTPUT_ROOT=${IMAGINAIRE_OUTPUT_ROOT:-/data/user/jhe724/workspace/cosmos-predict2.5/outputs/droid_v21_match_ground_v2_iou50_vlm4wam}
...
# Needs: GT masks (supervision) + rawseg_ft (2048 WHAT) + ft (256 WHERE query).
TOLERATE = 0.01
...
checks = {
    ...
    "target_features_ft": lambda s: (d / "target_features_ft" / f"{s}.pt").exists(),
}
ok = True
for name, fn in checks.items():
    ...
    ok = ok and frac <= TOLERATE
if not ok:
    sys.exit(1)
sample = active[0].stem
for dirname, dim in (("target_features_rawseg_ft", 2048), ("target_features_ft", 256)):
...
SAVE_ITER=${SAVE_ITER:-490}
SAMPLE_ITER=${SAMPLE_ITER:-245}
...
EXPERIMENT=${EXPERIMENT:-predict2_video2world_training_2b_droid_success_v21_match_ground_v2}
JOB_NAME=${JOB_NAME:-2b_mgv2_iou50_49f_bs2accum8_gbs128_10ep1470}
...
echo "=== TRAIN match-ground (mask-free); experiment=${EXPERIMENT}; per_gpu_batch=${BATCH_SIZE}; grad_accum=${GRAD_ACCUM_ITER}; global_batch=$((BATCH_SIZE * 8 * GRAD_ACCUM_ITER)); max_iter=${MAX_ITER}; job_name=${JOB_NAME}; output_root=${IMAGINAIRE_OUTPUT_ROOT} ==="
