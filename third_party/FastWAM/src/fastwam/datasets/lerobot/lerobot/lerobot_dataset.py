...
from .column_compat import stack_hf_column
...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # Check timestamps
        timestamps = stack_hf_column(self.hf_dataset["timestamp"]).numpy()
        episode_indices = stack_hf_column(self.hf_dataset["episode_index"]).numpy()
    ...
    def _get_query_timestamps(
        ...
        for key in self.meta.video_keys:
            if query_indices is not None and key in query_indices:
                ...
                query_timestamps[key] = stack_hf_column(timestamps).tolist()
            else:
    ...
    def _query_hf_dataset(self, query_indices: dict[str, list[int]]) -> dict:
        return {
            key: stack_hf_column(self.hf_dataset.select(q_idx)[key])
    ...
    def _query_hf_dataset_fast(self, query_indices: dict[str, list[int]]) -> dict:
        ...
        for key, q_idx in query_indices.items():
            if key not in self.meta.video_keys :
                ...
                result[key] = stack_hf_column(selected_data[key])
        return result
    ...
    def get_episode_data(self, episode_id: int) -> dict:
        ...
        res = {key : stack_hf_column(selected_data[key]) for key in res_keys}
        return res
