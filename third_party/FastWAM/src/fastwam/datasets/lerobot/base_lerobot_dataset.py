...
def _normalize_positive_fps(value) -> float:
    if isinstance(value, bool) or not isinstance(value, Real):
        raise TypeError(f"fps must be a real number, got {value!r}")
    value = float(value)
    if value <= 0 or not math.isfinite(value):
        raise ValueError(f"fps must be positive and finite, got {value}")
    return value
