...
TAG_PREFIX="${TAG_PREFIX:-plus_supp_$(date +%Y%m%d_%H%M%S)}"
REVERSE_MISSING="${REVERSE_MISSING:-1}"
MAX_PAIRS_PER_PROC="${MAX_PAIRS_PER_PROC:-0}"
DRY_RUN="${DRY_RUN:-0}"
...
if [ -z "$OUT" ]; then
  echo "[plus-supp] ERROR: OUT is empty and /data/users/junjie/current_eval_plus_out.txt is unavailable" >&2
  exit 2
fi
...
NGPU="${#GPUS[@]}"
if [ "$NGPU" -le 0 ]; then
  echo "[plus-supp] ERROR: empty GPU_LIST=$GPU_LIST" >&2
  exit 2
fi
...
for path in sorted(glob.glob(os.path.join(out, "results_*.json")) + glob.glob(os.path.join(out, "results_partial_*.json"))):
    ...
    for row in data.get("by_task", []):
        ...
        if trials >= 1:
            seen.add((row["suite"], int(row["task_id"])))
...
print(
    f"[plus-supp] expected={len(expected)} seen={len(seen)} missing={len(missing)} "
    f"nproc={nproc} reverse={reverse} max_pairs_per_proc={max_pairs_per_proc} launched_pairs={launched}"
)
PYEOF

PAIR_COUNT="$(wc -l < "$MISSING_FILE" | tr -d ' ')"
if [ "$PAIR_COUNT" -le 0 ]; then
  echo "[plus-supp] nothing missing"
  exit 0
fi
if [ "$DRY_RUN" = "1" ]; then
  echo "[plus-supp] dry run only: missing_file=$MISSING_FILE shard_dir=$SHARD_DIR"
  exit 0
fi

SHARD_COUNT="$(find "$SHARD_DIR" -maxdepth 1 -name 'shard_*.txt' | wc -l | tr -d ' ')"
echo "[plus-supp] launching $SHARD_COUNT procs on gpus=$GPU_LIST out=$OUT tag=$TAG_PREFIX"
...
for idx in $(seq 0 $((SHARD_COUNT - 1))); do
  ...
  echo "[plus-supp] launch idx=$idx gpu=$gpu pairs=$(awk -F, '{print NF}' <<< "$pairs") log=$log"
  (
...
echo "[plus-supp] done failed_procs=$failed"
exit 0
