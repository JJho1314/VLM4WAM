...
STANDARD_ROUNDS="${STANDARD_ROUNDS:-12}"
...
echo "[standard-tasks-backfill] run=$RUN_ID start=$START_STEP min=$MIN_STEP nproc=$STANDARD_NPROC gpus=$STANDARD_GPU_LIST tasks_per_job=$TASKS_PER_JOB rounds=$STANDARD_ROUNDS"

step="$START_STEP"
while [ "$step" -ge "$MIN_STEP" ]; do
  existing="$(complete_existing "$step")"
  if [ -n "$existing" ]; then
    echo "[standard-tasks-backfill] step=$step already complete: $existing"
    step=$((step - STEP_INTERVAL))
    continue
  fi

  out="$REPO/evaluate_results/${RUN_ID}_step$(printf '%06d' "$step")_libero_standard50_tasks_${TS}"
  log="$REPO/evaluate_results/auto_eval_logs/standard_tasks_step${step}_${TS}.log"
  mkdir -p "$out"
  echo "$out" > /data/users/junjie/current_eval_standard_out.txt
  echo "$log" > /data/users/junjie/current_eval_standard_log.txt
  echo "[standard-tasks-backfill] step=$step out=$out log=$log"

  round=1
  while [ "$round" -le "$STANDARD_ROUNDS" ]; do
    read -r units succ trials bad < <(parse_standard "$out")
    echo "[standard-tasks-backfill] $(date -u '+%Y-%m-%dT%H:%M:%SZ') step=$step round=$round units=$units/2000 succ=$succ trials=$trials bad=$bad" | tee -a "$log"
    if [ "$units" -ge 2000 ] && [ "$trials" -ge 2000 ] && [ "$bad" -eq 0 ]; then
      write_standard_summary "$out" | tee -a "$log"
      break
    fi
    tag="standard_tasks_step${step}_r${round}_$(date +%Y%m%d_%H%M%S)"
    OUT="$out" STEP="$step" NIS="$NIS" NPROC="$STANDARD_NPROC" GPU_LIST="$STANDARD_GPU_LIST" \
      ...
      bash scripts/supplement_standard_tasks_missing.sh 2>&1 | tee -a "$log"
    round=$((round + 1))
  done
  ...
  if [ "$units" -lt 2000 ] || [ "$trials" -lt 2000 ] || [ "$bad" -ne 0 ]; then
    ...
    echo "[standard-tasks-backfill] ERROR: incomplete step=$step units=$units trials=$trials bad=$bad" | tee -a "$log" >&2
    exit 3
  fi
  step=$((step - STEP_INTERVAL))
done

echo "[standard-tasks-backfill] complete"
