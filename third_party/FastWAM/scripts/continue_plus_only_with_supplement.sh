...
SUPP_GPU_LIST="${SUPP_GPU_LIST:-0,1,2}"
SUPP_NPROC_SEQUENCE="${SUPP_NPROC_SEQUENCE:-24 12 6 3 1}"
SUPP_ROUNDS="${SUPP_ROUNDS:-8}"
...
if [ -z "$FIRST_OUT" ]; then
  echo "[continue-plus-only] ERROR: FIRST_OUT is empty" >&2
  exit 2
fi
...
supplement_until_complete() {
  ...
  for round in $(seq 1 "$SUPP_ROUNDS"); do
    ...
    echo "[continue-plus-only] $(date -u '+%Y-%m-%dT%H:%M:%SZ') step=$step round=$round before tasks=$tasks succ=$succ trials=$trials bad=$bad out=$out"
    ...
    for nproc in $SUPP_NPROC_SEQUENCE; do
      local tag
      tag="plus_only_step${step}_r${round}_n${nproc}_$(date +%Y%m%d_%H%M%S)"
      echo "[continue-plus-only] supplement step=$step round=$round nproc=$nproc tag=$tag"
      ...
      read -r tasks succ trials bad < <(parse_plus "$out")
      echo "[continue-plus-only] after supplement step=$step nproc=$nproc tasks=$tasks succ=$succ trials=$trials bad=$bad"
      if [ "$tasks" -ge "$EXPECTED" ] && [ "$trials" -ge "$EXPECTED" ] && [ "$bad" -eq 0 ]; then
        write_plus_summary "$out"
        return 0
      fi
  ...
  echo "[continue-plus-only] ERROR: plus incomplete after supplements step=$step out=$out" >&2
  return 3
}
...
run_empty_step() {
  local step="$1"
  local ts="$2"
  local out="$REPO/evaluate_results/${RUN_ID}_step$(printf '%06d' "$step")_libero_plus_full10030_plusonly_${ts}"
  local log="$REPO/evaluate_results/auto_eval_logs/plus_only_step${step}_${ts}.log"
  mkdir -p "$out"
  echo "$out" > /data/users/junjie/current_eval_plus_out.txt
  echo "$log" > /data/users/junjie/current_eval_plus_log.txt
  echo "[continue-plus-only] new step=$step out=$out log=$log"
  supplement_until_complete "$out" "$step" > "$log" 2>&1
}

TS="$(date +%Y%m%d_%H%M%S)"
echo "[continue-plus-only] first_step=$FIRST_STEP first_out=$FIRST_OUT next=$START_STEP_AFTER min=$MIN_STEP gpus=$SUPP_GPU_LIST nproc_seq='$SUPP_NPROC_SEQUENCE'"
...
echo "[continue-plus-only] all queued plus-only evals complete"
