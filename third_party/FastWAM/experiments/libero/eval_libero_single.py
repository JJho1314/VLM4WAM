...
def _add_online_semantic_inference_inputs(
    infer_kwargs: dict,
    *,
    ...
    """Add raw planner inputs only for the dedicated online FastWAM mode.

    Signature checks keep legacy inference APIs unchanged.  Video timing is an
    explicit evaluation input: silently guessing it here would desynchronise
    semantic RoPE from the sampled clip timing.
    """
    if not bool(cfg.model.get("online_semantic_planner", False)):
        return infer_kwargs
    ...
    *,
    ...
    with torch.no_grad():
        if visualize_future_video:
            ...
            _add_online_semantic_inference_inputs(
                infer_kwargs,
