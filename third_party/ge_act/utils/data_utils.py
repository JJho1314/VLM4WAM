...
def get_latents(vae,
    ...
    encode_future: when False, skip the (expensive) Wan-VAE encode of the future clip and
        return a correctly-shaped zeros tensor. Safe in action training with noisy_video=True:
        the future latents are overwritten by pure noise (ss=1) and carry no video loss
        (train_mode action_only/action_full), so their content is never used — encoding the
        (repeated dummy) future frames is ~32% of a step wasted.
    Returns:
    ...
    if encode_future:
        ...
        f = (video.shape[2] - 1) // vae.temporal_compression_ratio + 1
        h = video.shape[3] // vae.spatial_compression_ratio
        w = video.shape[4] // vae.spatial_compression_ratio
        # match the encode_future=True path: latents.to(dtype=None) keeps the VAE's dtype
        # (bf16), so zeros must too — else the concat with mem_latents mixes float32/bf16
        # and the transformer's F.linear raises a dtype mismatch.
        out_dtype = dtype if dtype is not None else vae.dtype
        video_latents = torch.zeros(
            video.shape[0], vae.z_dim, f, h, w, device=device, dtype=out_dtype
        )
        video_latents = _pack_latents(video_latents, patch_size, patch_size_t)
